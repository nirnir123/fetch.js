...
/*JSON.stringify(localStorage.getItem('costs'));


async function fetchCosts() {
    try {
        return localStorage.getItem('costs');
    } catch {
        alert("couldn't fetch costs");
    }
}

async function getRelevantCosts(month, year) {
    let costs = fetchCosts().then(costs.filter((cost) => {
        return JSON.stringify(cost.month == month && cost.year == year);
    }))
    return costs;
}*/
